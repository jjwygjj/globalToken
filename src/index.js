...
program
  ...
  .option(
    "-m, --mode <mode>",
    "The token is sent to the remote mode. file and redis",
  )
  .option("-k, --redis_key <redis_key>", "Redis set_key value")
  .option(
    "-U, --redis_url <redis_url>",
...
const redis_key = program.redis_key
console.log("args:", {
  mode,
  token_url,
  user,
...
function createSh(data, pathfile) {
  ...
  data.map(x => {
    const ip = x.ip
    const user = x.user
    deploy_string += `rsync -azP ${path.join(
      __dirname,
      "..",
      "token.txt",
    )} ${user}@${ip}:${pathfile}/token.txt \n`
  })
  saveFile("deploy.sh", deploy_string)
}
function deploy() {
  exec(
    `sh ${path.join(__dirname, "..", "deploy.sh")}`,
    (error, stdout, stderr) => {
      if (error) {
...
function deploy_redis(data) {
  ...
  client.on("ready", function() {
    client.set(redis_key || "token", data)
    process.exit(0)
  })
  return
}
